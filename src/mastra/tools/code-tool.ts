...
import codeReview from "../utils/expectedResult";
...
export const codeTool = createTool({
  ...
  execute: async ({ context }) =>{
    ...
    try {
      result = await someCodeExecutionFunction(code);
    } catch (error) {
...
function someCodeExecutionFunction(code: string): string | PromiseLike<string> {
